...
public class CreateUtilisateursValidator : AbstractValidator<createUtilisateur>
{
    public CreateUtilisateursValidator()
    {
		...
		RuleFor(v => v.JobInTech)
	        .NotNull().WithMessage("'JobInTech' cannot be null.");

        RuleFor(v => v.Isblock)
            .NotNull().WithMessage("'Isblock' cannot be null.");
        ...
        RuleFor(v => v.PasswordHash)
            ...
            .Matches(@"(?=(.*[!@#\$%^&*(),.?""\\:{}|<>]){2})").WithMessage("Password must contain at least 2 special characters.");
    }
}